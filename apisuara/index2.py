...
def encrypt():
    # Gunakan request.form untuk mendapatkan teks dan request.files untuk mendapatkan file
    ...
    input_audio_file_path = os.path.join(temp_folder, input_audio_file.filename)
    input_audio_file.save(input_audio_file_path)

    user_key_cipher = shift_cipher_13(user_key_plain).encode("utf-8")
    output_text_file = f"{output_file_name}.txt"

    start_time = time.time()
    encrypt_audio(input_audio_file_path, output_text_file, user_key_cipher)
    end_time = time.time()
